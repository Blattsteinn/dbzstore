...
> **Workflow**: The user runs the test suite themselves (`bin/rails test`). **Do NOT run tests automatically** after every change/commit, and don't kick off test runs as part of finishing a task unless the user explicitly asks.
...
| `products_controller.rb` | `index` (visible products filtered by `params[:game]`, ordered by priority, tracks `"Viewed products"` Ahoy event) / `show` (any visibility but redirects unless live; sets contact markdown `@text` — the show view renders its own labeled contact markdown instead since 2026-08-14; tracks `"Viewed product"` event with `product`/`title`/`game` properties) / admin CRUD + `product_visibility` toggle (live↔hidden) + `duplicate_product` (deep-copy + variants, appends " Copy"). **Dead code:** `show_hero/minimal/split/card/gallery`. **No HTTP caching** — `stale?`/`fresh_when` conditional GETs were removed 2026-08-23 so stock changes (variant `decrement!`) and template edits always re-render (etags keyed on product `updated_at` don't cover variant stock or template digests in Rails 8). |
...
| `games_controller.rb` | `index` only — `Game.all` (no `fresh_when` since 2026-08-23) |
